...
// @version      11.14
// @description  yt-dlp + Persistent SSE + Hover Download + Share Link Catcher + Minimizable Shorts Shelf
// @match        *://*.youtube.com/*
// @noframes
...
// 1. Mencegah Injeksi Ganda (Lock agar tidak numpuk saat pindah halaman SPA)
if (window.__ytdl_initialized) return;
window.__ytdl_initialized = true;

// 2. Bersihkan elemen lama jika script di-save/direfresh paksa oleh Tampermonkey
...
// Variabel Global untuk menyimpan data Scan terakhir agar bisa difilter ulang
let lastScannedFiles = [];

// Tracker Cadangan: Secara diam-diam merekam link video apa yang sedang disentuh/dilihat kursor
...
function trackHover(e) {
    ...
    if (card) {
        ...
        if (a) window.__ytdlTargetUrl = a.href;
...
// Route YouTube CDN thumbnail URLs melalui proxy
...
/* FAB CONTAINER (Download & Library) - Diangkat sedikit agar tidak nabrak Live Chat */
#ytdl-fab-container {position:fixed;bottom:85px;right:20px;display:flex;flex-direction:column;gap:12px;z-index:999999;}

/* TOMBOL KOTAK, MERAH SAMA, TANPA HOVER EFEK */
...
#ytdl-fab-dl {display:none;} /* Default hidden, dimunculkan lewat JS jika di /watch */
...
/* HIDE SHARE DIALOG SEMENTARA SAAT MENANGKAP LINK (NUKLIR) */
...
/* HOVER DOWNLOAD BUTTON (Di Thumbnail Beranda) - Dibuat Merah Solid */
.ytdl-hover-dl {
    position:absolute;
    top:8px;
    right:8px;
    ...
    z-index:99;
    opacity:0;
    transition:opacity 0.2s ease, transform 0.1s ease, background 0.2s;
    display:flex;
    align-items:center;
    gap:5px;
}
...
ytd-compact-video-renderer:hover .ytdl-hover-dl {
    opacity:1;
}
...
/* HISTORY & SCAN LISTS (UNIFIED) */
#h-list, #scan-list{flex:1;overflow-y:auto;min-height:0;padding-right:2px}

/* GRID CONTAINER STYLES (MENGGUNAKAN FLEX WRAP, ALIGN START UNTUK MENCEGAH STRETCH) */
...
/* Grid folder group (Flex wrap - Align Start mencegah stretch tinggi) */
...
/* =========================================================================
   NEW SHORTS SHELF (MUNCUL DI FILTER "SEMUA", SCROLL SAMPING)
========================================================================= */
...
/* Menghilangkan shorts shelf bila header-nya collapsed */
...
/* Paksa card di dalam Shelf menjadi tegak (grid style mutlak) */
.shorts-shelf-container .v-card {
    flex-direction: column !important;
    flex: 0 0 110px !important;
    width: 110px !important;
    height: 195px !important; /* 110 * 16/9 */
}
.lib-maximized .shorts-shelf-container .v-card {
...
.shorts-shelf-container .cb {
    position: absolute !important; inset: 0 !important; padding: 10px !important;
    background: linear-gradient(to top,rgba(0,0,0,.95) 0%,rgba(0,0,0,.6) 50%,transparent 100%) !important; opacity: 0;
}
...
/* =========================================================================
   GRID CARD (NORMAL & SHORTS DI FILTER LAINNYA)
========================================================================= */
#h-list.grid .v-card, #scan-list.grid .v-card {
    flex-direction: column;
    height: auto;
    min-height: min-content;
    cursor: pointer;
    width: calc(50% - 5px); /* Pas 2 kolom + margin */
...
/* Aspect Rasio Normal Video */
#h-list.grid .v-card:not(.shorts-card), #scan-list.grid .v-card:not(.shorts-card) { aspect-ratio: 16/9; }

/* Aspect Rasio Shorts di Grid Biasa */
#h-list.grid .v-card.shorts-card, #scan-list.grid .v-card.shorts-card {
...
/* Delete Button (Library) & Add Button (Scan) - Grid */
...
#scan-list.grid .sc-add:hover{background:rgba(0,100,0,.9)!important}
#scan-list.grid .sc-add.in-lib{background:rgba(0,50,0,.9);color:#0f0;cursor:default}
...
/* Delete Button (Library) & Add Button (Scan) - List */
...
#scan-list.list .sc-add:hover{color:#0f0;background:rgba(0,100,0,.75)}
#scan-list.list .sc-add.in-lib{background:rgba(0,50,0,.9);color:#0f0;cursor:default}
...
function checkFabVisibility() {
    if (fabDl) {
        // Hanya tampil di halaman video biasa atau shorts
...
// Attach listener to catch YouTube navigation changes
['yt-navigate-finish', 'yt-page-data-updated'].forEach(evt => {
    window.addEventListener(evt, checkFabVisibility);
});
// Execute on load
checkFabVisibility();
...
const hoverObserver = new MutationObserver(() => {
    // Cari elemen ytd-thumbnail yang belum dipasang tombol kita
    ...
    thumbnails.forEach(thumb => {
        ...
        btn.onclick = (e) => {
            ...
            e.stopPropagation(); // Mencegah event klik menjalar ke thumbnail

            // Cari elemen induk terdekat untuk mengambil data video
            const renderer = thumb.closest('ytd-rich-item-renderer, ytd-video-renderer, ytd-grid-video-renderer, ytd-compact-video-renderer, ytd-reel-item-renderer');
            if (!renderer) return;
            ...
            // Tentukan apakah itu tipe Shorts
            const isShorts = url.includes('/shorts/');

            openQualityPopup(url, title, isShorts);
        };

        // Pasang tombol ke dalam kontainer thumbnail
...
const menuObserver = new MutationObserver(() => {
    ...
    menuLists.forEach(menuList => {
        ...
        // Buat elemen item list ala YouTube Baru, dikasih warna merah agar stand out
        ...
        // FUNGSI PENANGKAP LINK SHARE DENGAN MODE INVISIBLE
        menuItem.onclick = (e) => {
            e.stopPropagation();

            // 1. Cari tombol Share/Bagikan yang asli dari menu YouTube ini
            ...
            if (shareItem) {
                // Tambahkan class CSS Nuklir untuk menyembunyikan semua dialog
                document.body.classList.add('ytdl-hide-share');

                // Klik paksa tombol share asli YouTube
                const shareBtn = shareItem.querySelector('button');
                if (shareBtn) shareBtn.click();
                else shareItem.click();

                // Mulai mengintai dialog share sampai input link muncul
                let checkCount = 0;
                const checkShare = setInterval(() => {
                    ...
                    if (shareInput && shareInput.value) {
                        ...
                        const videoUrl = shareInput.value;

                        // Coba ambil judul (jika ada) di dialog share
                        ...
                        // Buka Pop-up Download kita
                        const isShorts = videoUrl.includes('/shorts/');
                        setTimeout(() => openQualityPopup(videoUrl, title, isShorts), 100);

                        // KEMBALIKAN LAYAR NORMAL DENGAN JEDA (agar animasi close-dialog YouTube tersembunyi sepenuhnya)
                        setTimeout(() => {
                            document.body.classList.remove('ytdl-hide-share');
                        }, 500);

                    } else if (checkCount > 30) {
                        // Timeout 3 detik jika dialog gagal/ngelag
                        clearInterval(checkShare);
                        setTimeout(() => document.body.classList.remove('ytdl-hide-share'), 500);

                        // Fallback: Gunakan tracker pantauan kursor jika Share gagal
                ...
                // Fallback instan jika tombol Share tidak ada sama sekali di menu tersebut
                if (window.__ytdlTargetUrl) {
                    document.dispatchEvent(new KeyboardEvent('keydown', { key:'Escape', bubbles:true }));
                    setTimeout(() => openQualityPopup(window.__ytdlTargetUrl, window.__ytdlTargetTitle, window.__ytdlTargetUrl.includes('/shorts/')), 150);
                } else {
        ...
        // Sisipkan ke dalam list menu (misal diletakkan di sebelum opsi paling akhir)
...
document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.onclick = () => {
        ...
        // Re-render konten sesuai tab yang aktif
        loadHistory(true);
        if (lastScannedFiles.length > 0) {
            renderScanList(lastScannedFiles);
        }
    };
});
...
fabDl.onclick = () => {
    if (location.pathname.startsWith('/watch') || location.pathname.startsWith('/shorts/')) {
        ...
        openQualityPopup(location.href, document.title, isShorts);
    }
};
...
/* ══════════════════════════════════════════════════════
   PEMBUATAN KARTU UNTUK LIBRARY & SCAN
══════════════════════════════════════════════════════ */
function createCardElement(item, shorts, asShelf, isScan) {
    ...
    card.innerHTML = `
        ...
        ${isScan ?
            `<button class="sc-add ${inLib ? 'in-lib' : ''}" title="${inLib ? 'Sudah di Library' : 'Tambah ke Library'}">${inLib ? '✓' : '+'}</button>` :
    ...
    // Scan Action (Add to Lib) vs Library Action (Delete)
    if (isScan) {
...
function renderHistory(hist) {
    ...
    // --- 1. MEMBUAT SHORTS SHELF KHUSUS UNTUK FILTER 'SEMUA' ---
    if (libraryFilter === 'all') {
        ...
        if (shortsItems.length > 0) {
            ...
            shelfGroup.className = 'folder-group';
            ...
            shelfHeader.innerHTML = `
                ...
                <span class="fg-name" style="color:#fff;">Rak Shorts</span>
                <span class="fg-count" style="background:rgba(0,0,0,0.5);">${shortsItems.length}</span>
                <span class="fg-toggle">▼</span>
            `;

            const shelfBody = document.createElement('div');
            shelfBody.className = `shorts-shelf-container${isCollapsed ? ' collapsed' : ''}`;

            // Logika Klik Minimize pada Rak Shorts
            shelfHeader.onclick = () => {
                const collapsed = !shelfHeader.classList.contains('collapsed');
                shelfHeader.classList.toggle('collapsed', collapsed);
                shelfBody.classList.toggle('collapsed', collapsed);

                const saved = JSON.parse(localStorage.getItem('ytdl_collapsed_folders') || '{}');
                if (collapsed) saved[shelfKey] = true;
                else delete saved[shelfKey];
                localStorage.setItem('ytdl_collapsed_folders', JSON.stringify(saved));
            };

            shortsItems.forEach(item => {
                shelfBody.appendChild(createCardElement(item, true, true, false));
            });

            shelfGroup.appendChild(shelfHeader);
            shelfGroup.appendChild(shelfBody);
            list.appendChild(shelfGroup);
        }
    }

    // --- 2. FOLDER GROUPS UNTUK VIDEO BIASA (Atau Semua di Filter Lain) ---
    if (videosToGroup.length > 0) {
        ...
        Object.entries(groups).forEach(([folderPath, group]) => {
            ...
            group.items.forEach(item => {
                body.appendChild(createCardElement(item, checkIsShorts(item), false, false));
            });
...
function runScan() {
    ...
    GM_xmlhttpRequest({
        ...
        onload: (res) => {
            ...
            let files;
            try {
                files = JSON.parse(res.responseText);
                lastScannedFiles = files; // Simpan secara global
            } catch(_) {
...
// RENDER SCAN LIST MENGIKUTI STRUKTUR LIBRARY
function renderScanList(files) {
    ...
    if (!filtered.length) {
        list.innerHTML = `<p style="text-align:center;color:#333;margin-top:20px">Tidak ada ${libraryFilter === 'shorts' ? 'Shorts' : 'Video'} ditemukan dari hasil scan.</p>`;
        return;
    }
    ...
    // --- SHORTS SHELF KHUSUS FILTER 'SEMUA' ---
    if (libraryFilter === 'all') {
        const shortsItems = filtered.filter(item => checkIsShorts(item));
        videosToGroup = filtered.filter(item => !checkIsShorts(item));
        ...
        if (shortsItems.length > 0) {
            ...
            shelfHeader.innerHTML = `
                <span class="fg-icon">📱</span>
                <span class="fg-name" style="color:#fff;">Rak Shorts (Hasil Scan)</span>
                <span class="fg-count" style="background:rgba(0,0,0,0.5);">${shortsItems.length}</span>
                <span class="fg-toggle">▼</span>
            `;
            ...
            // Logika Klik Minimize pada Rak Shorts Scan
            shelfHeader.onclick = () => {
                const collapsed = !shelfHeader.classList.contains('collapsed');
                shelfHeader.classList.toggle('collapsed', collapsed);
                shelfBody.classList.toggle('collapsed', collapsed);
            ...
            shortsItems.forEach(item => {
                shelfBody.appendChild(createCardElement(item, true, true, true)); // isShorts=true, asShelf=true, isScan=true
            });
    ...
    if (videosToGroup.length > 0) {
        ...
        Object.entries(groups).forEach(([folderPath, group]) => {
            ...
            group.items.forEach(item => {
                body.appendChild(createCardElement(item, checkIsShorts(item), false, true)); // isScan = true
            });
