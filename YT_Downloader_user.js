...
// @version      11.27
// @description  yt-dlp + SSE + Hover + Share Catcher + Virtual Folders + Auto Playlist + Ultimate Clean + Pause/Resume + Trusted Types Fix + Close Player + Dismiss Card + True Fullscreen Fix + Video Resume
// @match        *://*.youtube.com/*
// @noframes
...
(function () {
    ...
    window.__ytdl_active_version = "11.27";
    ...
    styleEl.textContent = `
    @keyframes ytdl-fadein   { from{opacity:0} to{opacity:1} }
    @keyframes ytdl-slideup  { from{opacity:0;transform:translateY(18px) scale(0.97)} to{opacity:1;transform:translateY(0) scale(1)} }
    @keyframes ytdl-popin    { from{opacity:0;transform:translate(-50%,-50%) scale(0.88)} to{opacity:1;transform:translate(-50%,-50%) scale(1)} }
    @keyframes ytdl-cardin   { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }
    ...
    .ytdl-fab {width:48px;height:48px;background:#e00;color:#fff;border-radius:10px;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 4px 15px rgba(0,0,0,.5);font-size:20px;border:1px solid rgba(255,255,255,.2);user-select:none;transition:transform .15s ease, background .2s ease, box-shadow .2s ease;}
    .ytdl-fab:hover{transform:scale(1.1);background:#c00;box-shadow:0 6px 20px rgba(0,0,0,.7);}
    .ytdl-fab:active{transform:scale(0.93);}
    #ytdl-fab-dl {display:none;}
    #ytdl-fab-lib {position:relative;}
    #ytdl-fab-lib .badge {position:absolute;top:-6px;right:-6px;background:#fa0;color:#000;border-radius:50%;width:18px;height:18px;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;animation:ytdl-popin .2s ease;}
    ...
    .ytdl-hover-dl {
        ...
        opacity:0; transition:opacity 0.18s ease, transform 0.15s ease, background 0.2s;
        display:flex; align-items:center; gap:5px; transform:translateY(4px);
    }
    .ytdl-hover-dl:hover { background:#c00 !important; transform:translateY(0) scale(1.04) !important; }
    .ytdl-hover-dl:active { transform:scale(0.95) !important; }
    ...
    ytd-compact-video-renderer:hover .ytdl-hover-dl { opacity:1; transform:translateY(0); }

    /* ── MODAL TRUE FULLSCREEN FIX ──
     Y o*uTube creates transform/filter stacking contexts on body/html that break
     position:fixed. Solution: use a wrapper injected at documentElement level
     with a shadow-DOM-like isolation approach + aggressive CSS reset.
     */
    #ytdl-modal-backdrop {
    ...
    #ytdl-modal-backdrop.show {
    display: block !important;
    pointer-events: auto !important;
    animation: ytdl-fadein .22s ease forwards !important;
    }
    #ytdl-modal {
    ...
    #ytdl-popup{display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) scale(0.9);background:#181818;color:#fff;padding:22px;border-radius:14px;z-index:10000000;width:340px;box-shadow:0 0 40px rgba(0,0,0,.9);border:1px solid #3a3a3a;text-align:center;opacity:0;transition:opacity .2s ease, transform .2s ease;}
    #ytdl-popup.show{display:block;animation:ytdl-popin .22s cubic-bezier(0.34,1.56,0.64,1) forwards;}
    ...
    .q-btn{background:#2a2a2a;color:#fff;border:1px solid #444;padding:12px 8px;border-radius:8px;cursor:pointer;font-weight:700;font-size:13px;transition:background .18s ease, border-color .18s ease, transform .12s ease;display:flex;flex-direction:column;align-items:center;gap:3px}
    .q-btn:hover{background:#e00;border-color:#e00;transform:translateY(-2px);}
    .q-btn:active{transform:scale(0.96);}
    .q-btn .q-sz{font-size:10px;color:#aaa;font-weight:400;transition:color .18s}
    .q-btn:hover .q-sz{color:#ffc}
    #btn-dl-playlist{transition:background .2s ease, transform .12s ease;}
    #btn-dl-playlist:hover { background: #0c7 !important; transform:translateY(-1px); }
    #btn-dl-playlist:active { transform:scale(0.97); }
    ...
    #player-wrap{flex-shrink:0;transition:max-height .35s ease, opacity .35s ease, margin-bottom .35s ease}

    #player-toggle-bar{display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-shrink:0;cursor:pointer;user-select:none;padding:6px 10px;border-radius:8px;border:1px solid #1a1a1a;transition:background .2s ease, border-color .2s ease;}
    #player-toggle-bar:hover{background:#111;border-color:#333}
    #player-toggle-bar .ptb-title{flex:1;font-size:11px;color:#555;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transition:color .2s}
    #player-toggle-bar .ptb-title.playing{color:#3ea6ff}
    #player-toggle-bar .ptb-icon{font-size:14px;transition:transform .3s cubic-bezier(0.34,1.56,0.64,1);flex-shrink:0}
    #player-toggle-bar .ptb-icon.collapsed{transform:rotate(-90deg)}
    #player-toggle-bar .ptb-badge{background:#222;color:#555;font-size:9px;padding:2px 6px;border-radius:4px;flex-shrink:0;transition:background .25s, color .25s;}
    ...
    .dl-card{background:#111;border:1px solid #222;border-radius:10px;padding:12px;margin-bottom:10px;transition:opacity 0.3s ease, transform 0.3s ease, border-color .2s ease;animation:ytdl-cardin .25s ease forwards;}
    ...
    .dl-pause{background:none;border:1px solid #555;color:#fa0;border-radius:5px;padding:3px 10px;font-size:10px;cursor:pointer;transition:all .18s ease;white-space:nowrap}
    .dl-pause:hover{background:#fa0;border-color:#fa0;color:#000;transform:translateY(-1px);}
    .dl-pause:active{transform:scale(0.95);}
    ...
    .dl-cancel{background:none;border:1px solid #522;color:#f44;border-radius:5px;padding:3px 10px;font-size:10px;cursor:pointer;transition:all .18s ease;white-space:nowrap}
    .dl-cancel:hover{background:#e00;border-color:#e00;color:#fff;transform:translateY(-1px);}
    .dl-cancel:active{transform:scale(0.95);}

    .dl-dismiss{background:none;border:1px solid #333;color:#555;border-radius:5px;padding:3px 10px;font-size:10px;cursor:pointer;transition:all .18s ease;white-space:nowrap;margin-left:auto}
    .dl-dismiss:hover{background:#333;border-color:#555;color:#fff;transform:translateY(-1px);}
    .dl-dismiss:active{transform:scale(0.95);}
    ...
    .modal-btn-row{display:flex;gap:8px;justify-content:center;margin-top:10px;flex-shrink:0}
    .close-ytdl{background:#1a1a1a;color:#888;border:1px solid #333;padding:9px 36px;border-radius:20px;cursor:pointer;font-weight:700;transition:all .2s ease;}
    .close-ytdl:hover{background:#333;color:#fff;transform:translateY(-1px);}
    .close-ytdl:active{transform:scale(0.96);}
    #close-ytdl{background:#1a1a1a;color:#888;border:1px solid #333;padding:9px 36px;border-radius:20px;cursor:pointer;font-weight:700;transition:all .2s ease;}
    #close-ytdl:hover{background:#333;color:#fff;transform:translateY(-1px);}
    #close-ytdl:active{transform:scale(0.96);}
    #close-player-btn{background:#1a1a1a;color:#888;border:1px solid #333;padding:9px 18px;border-radius:20px;cursor:pointer;font-weight:700;transition:all .2s ease;white-space:nowrap}
    #close-player-btn:hover{background:#c00;border-color:#e00;color:#fff;transform:translateY(-1px);}
    #close-player-btn:active{transform:scale(0.96);}
    ...
    .side-p.lib-maximized{position:fixed;inset:16px;border-radius:16px;z-index:10000000;padding:20px;box-shadow:0 0 60px rgba(0,0,0,.9);animation:ytdl-fadein .2s ease;}
    .lib-tabs{display:flex;gap:4px;margin-bottom:10px;flex-shrink:0}
    .lib-tab{flex:1;padding:7px 0;background:#1a1a1a;border:1px solid #222;border-radius:7px;color:#555;font-size:12px;font-weight:700;cursor:pointer;text-align:center;transition:background .2s ease, color .2s ease, border-color .2s ease, transform .12s ease;}
    .lib-tab:hover:not(.active){background:#222;color:#aaa;transform:translateY(-1px);}
    .lib-tab:active{transform:scale(0.96);}
    ...
    .vbtn{background:#1a1a1a;border:1px solid #333;color:#888;padding:5px 10px;border-radius:6px;cursor:pointer;font-size:15px;transition:all .18s ease;display:flex;align-items:center;}
    .vbtn.active{background:#e00;border-color:#e00;color:#fff}
    .vbtn-txt {font-size:13px; font-weight:bold;}
    .vbtn:hover{background:#333;color:#fff;transform:translateY(-1px);}
    .vbtn:active{transform:scale(0.92);}

    .maximize-btn{background:#1a1a1a;border:1px solid #333;color:#888;padding:5px 10px;border-radius:6px;cursor:pointer;font-size:13px;transition:all .18s ease;white-space:nowrap}
    .maximize-btn:hover{background:#333;color:#fff;transform:translateY(-1px);}
    .maximize-btn:active{transform:scale(0.92);}

    .filter-row{display:flex;gap:4px;margin-bottom:14px;flex-shrink:0; padding-bottom:14px; border-bottom:1px solid #222;}
    .filter-btn{flex:1;padding:5px 0;background:#1a1a1a;border:1px solid #222;border-radius:6px;color:#555;font-size:11px;font-weight:700;cursor:pointer;text-align:center;transition:background .18s ease, color .18s ease, border-color .18s ease, transform .12s ease;}
    .filter-btn:hover:not(.active){background:#222;color:#aaa;transform:translateY(-1px);}
    .filter-btn:active{transform:scale(0.95);}
    .filter-btn.active{background:#333;border-color:#555;color:#fff}

    .folder-group{margin-bottom:14px}
    .folder-group-header{display:flex;align-items:center;gap:6px;padding:6px 10px;background:#1a1a1a;border-radius:7px;margin-bottom:6px;cursor:pointer;user-select:none;border:1px solid #222;transition:background .2s ease, border-color .2s ease, transform .12s ease;}
    .folder-group-header:hover{background:#222;transform:translateX(2px);}
    .folder-group-header:active{transform:scale(0.99);}
    ...
    .folder-group-header .fg-count{background:#222;color:#555;font-size:9px;padding:2px 6px;border-radius:4px;flex-shrink:0;transition:background .2s, color .2s;}
    .folder-group-header .fg-toggle{font-size:10px;color:#444;transition:transform .3s cubic-bezier(0.34,1.56,0.64,1);flex-shrink:0}
    .folder-group-header.collapsed .fg-toggle{transform:rotate(-90deg)}
    .fg-del-btn { background:none; border:none; color:#f44; cursor:pointer; font-size:14px; margin-left:4px; padding:0 5px; border-radius:4px; transition:all 0.18s ease; }
    .fg-del-btn:hover { background:#f44; color:#fff; transform:scale(1.15); }
    .fg-del-btn:active { transform:scale(0.9); }
    ...
    #h-list.grid .folder-group-body, #scan-list.grid .folder-group-body {display:flex; flex-wrap:wrap; gap:10px; padding:2px; align-items:flex-start; overflow:hidden; max-height:5000px; opacity:1; transition:max-height 0.38s cubic-bezier(0.4,0,0.2,1), opacity 0.28s ease, padding 0.28s ease;}
    #h-list.list .folder-group-body, #scan-list.list .folder-group-body {display:flex;flex-direction:column;gap:8px;padding:2px; overflow:hidden; max-height:5000px; opacity:1; transition:max-height 0.38s cubic-bezier(0.4,0,0.2,1), opacity 0.28s ease, padding 0.28s ease;}
    .folder-group-body.collapsed { max-height:0 !important; opacity:0 !important; padding-top:0 !important; padding-bottom:0 !important; }

    .shorts-shelf-container { display: flex !important; flex-wrap: nowrap !important; overflow-x: auto !important; gap: 10px; padding: 2px 2px 10px 2px; scroll-behavior: smooth; align-items: center; max-height:400px; opacity:1; transition: max-height 0.38s cubic-bezier(0.4,0,0.2,1), opacity 0.28s ease; }
    .shorts-shelf-container.collapsed { max-height:0 !important; opacity:0 !important; overflow:hidden !important; padding:0 !important; }
    ...
    .v-card{display:flex;background:#111;border-radius:10px;border:1px solid #222;overflow:hidden;position:relative;transition:border-color .2s ease, transform .2s ease, opacity .2s ease, box-shadow .2s ease;}
    .v-card:hover{border-color:#555;transform:translateY(-2px);box-shadow:0 4px 16px rgba(0,0,0,.5);}
    ...
    #h-list.grid .thumb-wrap img, #scan-list.grid .thumb-wrap img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .35s ease}
    #h-list.grid .v-card:hover .thumb-wrap img, #scan-list.grid .v-card:hover .thumb-wrap img{transform:scale(1.07)}
    ...
    #h-list.grid .del-b, #scan-list.grid .sc-add{position:absolute;top:6px;left:6px;z-index:5;background:rgba(0,0,0,.75);border:none;color:#fff;border-radius:50%;width:24px;height:24px;display:flex;align-items:center;justify-content:center;font-size:11px;padding:0;cursor:pointer;opacity:0;transition:opacity .18s ease, transform .15s ease;line-height:1}
    #h-list.grid .v-card:hover .del-b, #scan-list.grid .v-card:hover .sc-add{opacity:1;transform:scale(1);}
    #h-list.grid .del-b:hover{background:rgba(180,0,0,.9)!important;transform:scale(1.15)!important;}
    #h-list.grid .sc-add:hover{background:rgba(0,100,0,.9)!important;transform:scale(1.15)!important;}
    #h-list.grid .sc-add.in-lib{background:rgba(0,50,0,.9);color:#0f0;cursor:default}
    #h-list.grid .cb, #scan-list.grid .cb{position:absolute;inset:0;z-index:3;display:flex;flex-direction:column;justify-content:flex-end;padding:10px;background:linear-gradient(to top,rgba(0,0,0,.95) 0%,rgba(0,0,0,.6) 50%,transparent 100%);opacity:0;transition:opacity .22s ease;pointer-events:none;box-sizing:border-box}
    ...
    #h-list.list .thumb-wrap img, #scan-list.list .thumb-wrap img{width:100%;height:100%;object-fit:cover;display:block;transition:transform .35s ease;}
    #h-list.list .v-card:hover .thumb-wrap img, #scan-list.list .v-card:hover .thumb-wrap img{transform:scale(1.06);}
    ...
    #h-list.list .del-b, #scan-list.list .sc-add{position:absolute;top:6px;right:6px;background:rgba(0,0,0,.55);border:none;color:#aaa;font-size:12px;padding:3px 5px;cursor:pointer;transition:all .15s ease;border-radius:4px;line-height:1;z-index:2}
    #h-list.list .del-b:hover{color:#f44;background:rgba(180,0,0,.75);transform:scale(1.1);}
    #h-list.list .sc-add:hover{color:#0f0;background:rgba(0,100,0,.75);transform:scale(1.1);}
    ...
    .play-b, .sc-add-btn{background:#3ea6ff;border:none;color:#fff;padding:5px 12px;border-radius:5px;cursor:pointer;font-size:10px;font-weight:700;transition:background .15s ease, transform .12s ease;}
    .play-b:hover, .sc-add-btn:hover{background:#1a8fd1;transform:translateY(-1px);}
    .play-b:active, .sc-add-btn:active{transform:scale(0.95);}
    .del-b{cursor:pointer;border:none;background:none;transition:color .15s, transform .12s;}

    #scan-panel{display:none;flex-direction:column;flex:1;overflow:hidden;min-height:0}
    #scan-panel.show{display:flex;animation:ytdl-fadein .18s ease;}
    ...
    .folder-item{display:flex;align-items:center;gap:6px;background:#1a1a1a;border:1px solid #222;border-radius:6px;padding:6px 10px;margin-bottom:4px;font-size:11px;color:#aaa;transition:background .15s, border-color .15s;}
    .folder-item:hover{background:#222;border-color:#333;}
    ...
    .folder-item .frem{background:none;border:none;color:#f44;cursor:pointer;font-size:13px;flex-shrink:0;transition:transform .15s, color .15s;}
    .folder-item .frem:hover{transform:scale(1.2);}

    .add-row{display:flex;gap:6px;margin-bottom:8px;flex-shrink:0}
    .add-row input{flex:1;background:#1a1a1a;border:1px solid #222;border-radius:6px;padding:7px 10px;color:#fff;font-size:11px;outline:none;transition:border-color .2s ease;}
    .add-row input:focus{border-color:#e00}

    .sbtn{background:#1a1a1a;border:1px solid #333;color:#ccc;border-radius:6px;padding:7px 12px;cursor:pointer;font-size:11px;font-weight:700;white-space:nowrap;transition:all .18s ease;flex-shrink:0}
    .sbtn:hover{background:#333;color:#fff;transform:translateY(-1px);}
    ...
    .resume-badge { cursor:pointer; transition: background .15s ease, transform .12s ease !important; }
    .resume-badge:hover { background:#0c6 !important; color:#fff !important; transform:translateY(-1px) !important; }
    `;
    ...
    // ── MODAL ──────────────────────────────────────────────────────────────────
    // Backdrop is appended to <html> (documentElement) to escape YouTube's
    // transform/filter stacking contexts that break position:fixed on body.
    const modalBackdrop = el('div', { id: 'ytdl-modal-backdrop' });
    ...
    // Button row: Close Player + Tutup Modal
    const modalBtnRow = el('div', { class: 'modal-btn-row' });
    const closePlayerBtn = el('button', { id: 'close-player-btn', textContent: '🗑 Tutup Player' });
    const closeBtn = el('button', { id: 'close-ytdl', class: 'close-ytdl', textContent: '✕ TUTUP' });
    modalBtnRow.appendChild(closePlayerBtn);
    modalBtnRow.appendChild(closeBtn);
    mainP.appendChild(modalBtnRow);
    ...
    // Append to <html> element to escape all YouTube stacking contexts
    ...
    document.getElementById('close-player-btn').onclick = () => {
        ...
        updateToggleBar('', Object.keys(_w.__ytdlProg || {}).length);
    };

    /* ══════════════════════════════════════════════════════
     V I*SIBILITY LOGIC FOR DOWNLOAD FAB
     ══════════════════════════════════════════════════════ */
    ...
    /* ══════════════════════════════════════════════════════
     I N*JEKSI TOMBOL HOVER DI BERANDA
     ══════════════════════════════════════════════════════ */
    ...
    /* ══════════════════════════════════════════════════════
     S C*ALE LOGIC — hanya scale konten di dalam panel,
     bukan modal wrapper atau grid layout.
     Gunakan CSS zoom pada .main-p dan .side-p saja.
     ══════════════════════════════════════════════════════ */
    function applyScale() {
        const mainPEl  = document.getElementById('main-p');
        const sidePEl  = document.querySelector('.side-p');
        if (!mainPEl || !sidePEl) return;
        // zoom tidak merusak layout grid dan tidak mempengaruhi posisi fixed modal
        const z = uiScale === 1 ? '' : String(uiScale);
        mainPEl.style.zoom = z;
        sidePEl.style.zoom = z;
    }
    applyScale();
    ...
    fabLib.onclick = () => { modalBackdrop.classList.add('show'); loadHistory(); };
    fabDl.onclick = () => {
    ...
    document.getElementById('close-ytdl').onclick = () => { modalBackdrop.classList.remove('show'); player.pause(); };
    document.getElementById('popup-cancel').onclick = () => popup.classList.remove('show');

    /* ══════════════════════════════════════════════════════
     S S*E PENGAMAT DOWNLOAD
     ══════════════════════════════════════════════════════ */
    ...
    _w.__ytdlSSE.onerror = () => {
        _w.__ytdlSSE.close();
        console.warn("YTDL: SSE ditutup karena error. Pastikan flag allow-insecure-localhost aktif.");
    };
    ...
    function renderDlPanel(data) {
        ...
        entries.forEach(([url, info]) => {
            ...
            htmlParts.push(`<div class="dl-foot">`);
            htmlParts.push(`<span class="dl-status" style="color:${statusColor}">${!done && !cancelled ? statusText : ''}</span>`);
            htmlParts.push(`<span>${done || cancelled ? fsz : ''}</span>`);

            if (active) {
                htmlParts.push(`<div class="dl-foot-btns">`);
                htmlParts.push(`<button class="dl-pause ${isPaused ? 'is-paused' : ''}" data-url="${url}" data-paused="${isPaused}">${isPaused ? '▶ Lanjutkan' : '⏸ Jeda'}</button>`);
                htmlParts.push(`<button class="dl-cancel" data-url="${url}">✕ Batalkan</button>`);
                htmlParts.push(`</div>`);
            }

            if (done || cancelled) {
                htmlParts.push(`<button class="dl-dismiss" data-url="${url}">✕ Tutup</button>`);
            }
            ...
            const dismissBtn = card.querySelector('.dl-dismiss');
            if (dismissBtn) {
                dismissBtn.onclick = function() {
                    ...
                    setTimeout(() => {
                        card.remove();
                        if (_w.__ytdlProg && _w.__ytdlProg[url]) {
                            delete _w.__ytdlProg[url];
                        }
                        const remaining = panel.querySelectorAll('.dl-card');
        ...
        panel.querySelectorAll('.dl-card').forEach(c => {
            ...
            if (!data[cardUrl] && c.style.opacity !== '0') c.remove();
        });
    }

    /* ══════════════════════════════════════════════════════
     R E*SUME / PROGRESS SYSTEM
     Key: ytdl_pos_<base64(fileName)>  →  { t: seconds, dur: total }
     ══════════════════════════════════════════════════════ */
    function resumeKey(fileName) {
        // Buat key pendek dari fileName
        try {
            return 'ytdl_pos_' + btoa(encodeURIComponent(fileName)).replace(/[^a-zA-Z0-9]/g, '').substring(0, 40);
        } catch(_) { return 'ytdl_pos_' + fileName.length; }
    }

    function saveProgress(fileName, t, dur) {
        if (!fileName || !t || t < 5) return; // jangan simpan jika baru mulai
        // Jika sudah di akhir (sisa < 10 detik atau > 97%), hapus
        if (dur > 0 && (t > dur - 10 || t / dur > 0.97)) {
            localStorage.removeItem(resumeKey(fileName));
            return;
        }
        localStorage.setItem(resumeKey(fileName), JSON.stringify({ t: Math.floor(t), dur: Math.floor(dur) }));
    }

    function getProgress(fileName) {
        try { return JSON.parse(localStorage.getItem(resumeKey(fileName))); } catch(_) { return null; }
    }
    ...
    // Simpan fileName yang sedang diputar agar timeupdate bisa akses
    let _currentPlayingFileName = '';
    let _saveProgressTimer = null;

    // Event listener global untuk simpan progress
    player.addEventListener('timeupdate', () => {
        if (!_currentPlayingFileName) return;
        // Throttle: simpan tiap 5 detik
        if (_saveProgressTimer) return;
        _saveProgressTimer = setTimeout(() => {
            _saveProgressTimer = null;
            saveProgress(_currentPlayingFileName, player.currentTime, player.duration || 0);
        }, 5000);
    });

    // Hapus progress saat video selesai
    player.addEventListener('ended', () => {
        if (_currentPlayingFileName) {
            clearProgress(_currentPlayingFileName);
            _currentPlayingFileName = '';
            // Update badge kartu terkait
            updateResumeBadges();
        }
    });

    function updateResumeBadges() {
        // Update semua badge resume yang ada di kartu library
        document.querySelectorAll('.resume-badge').forEach(b => {
            const fn = b.dataset.filename;
            if (!fn) return;
            const prog = getProgress(fn);
            if (prog && prog.t > 5) {
                b.textContent = `⏱ ${formatTime(prog.t)}`;
    ...
    function playFile(fileName, folderPath, title) {
        ...
        // Simpan dulu progress video lama
        if (_currentPlayingFileName && _currentPlayingFileName !== fileName) {
            saveProgress(_currentPlayingFileName, player.currentTime, player.duration || 0);
        }

        _currentPlayingFileName = fileName;
        player.src = src;

        // Restore posisi setelah metadata siap
        const savedPos = getProgress(fileName);
        if (savedPos && savedPos.t > 5) {
            const onMeta = () => {
                player.removeEventListener('loadedmetadata', onMeta);
                player.currentTime = savedPos.t;
                player.play();
                // Tampilkan toast notif resume
                showResumeToast(savedPos.t);
            };
    ...
    /* Toast notifikasi resume */
    function showResumeToast(seconds) {
        let toast = document.getElementById('ytdl-resume-toast');
        if (!toast) {
            toast = el('div', { id: 'ytdl-resume-toast', style: `
                position:absolute; bottom:80px; left:50%; transform:translateX(-50%);
                background:#1a3a1a; border:1px solid #0c6; color:#0f0;
                padding:8px 16px; border-radius:8px; font-size:12px; font-weight:700;
                z-index:10; white-space:nowrap; pointer-events:none;
                animation: ytdl-fadein .2s ease;
                ` });
            document.getElementById('main-p').style.position = 'relative';
            document.getElementById('main-p').appendChild(toast);
        }
        toast.textContent = `▶ Melanjutkan dari ${formatTime(seconds)}`;
    ...
    /* ══════════════════════════════════════════════════════
     P E*MBUATAN KARTU (DENGAN DRAG AND DROP)
══════════════════════════════════════════════════════ */
    function createCardElement(item, shorts, asShelf, isScan) {
        ...
        // Badge resume — tampil jika ada saved position
        const prog = !isScan ? getProgress(item.fileName) : null;
        if (prog && prog.t > 5) {
            const resumeBadge = el('span', {
                class: 'resume-badge tag',
                style: 'background:#1a3a1a;color:#0c6;cursor:pointer;',
                textContent: `⏱ ${formatTime(prog.t)}`
            });
            resumeBadge.dataset.filename = item.fileName;
            resumeBadge.title = `Lanjutkan dari ${formatTime(prog.t)}`;
            resumeBadge.onclick = (e) => {
                e.stopPropagation();
                playFile(item.fileName, item.folderPath || null, item.title);
            };
            cardActs.appendChild(resumeBadge);
        }
        ...
        playBtn.onclick = () => {
            if (isScan) {
                // Scan panel: pakai streamKey, resume via fileName jika ada
                if (_currentPlayingFileName && _currentPlayingFileName !== item.fileName) {
                    saveProgress(_currentPlayingFileName, player.currentTime, player.duration || 0);
                }
                _currentPlayingFileName = item.fileName || item.title;
                document.getElementById('p-title').textContent = item.title;
                player.src = `${API}/api/stream/${item.streamKey}`;
                const savedPos = getProgress(_currentPlayingFileName);
                if (savedPos && savedPos.t > 5) {
    ...
    setInterval(() => {
        ...
        prevKeys = keys;
        if (modalBackdrop.classList.contains('show')) loadHistory();
